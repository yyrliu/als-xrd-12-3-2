...
DEFAULT_OUTPUT_ROOT = Path(
    r"G:\Shared drives\Sutter-Fella Lab\ALS_Beamtimes\2026\Yi-Ru_Feb2026\processed\peak_tracking_workflow_outputs_snr"
)


def _mabai_peaks() -> list[tuple]:
    """MBAI peaks with validation and adaptive bounds."""
    return [
        (7.23, 2, (-1, 0), "2D (002)", {
...
def _meombai_peaks() -> list[tuple]:
    """MeOMBAI peaks with validation. Adaptive bounds ONLY for stable peaks."""
    return [
        (6.0, 0.5, (-1, 0), "2D (002)", {
...
def _clmbai_peaks() -> list[tuple]:
    """ClMBAI peaks with validation and adaptive bounds."""
    return [
        (6.4, 1, (-1, 25), "2D (002)", {
            "drift_tolerance_deg": 0.05,
            "fitter": {"kind": "voigt_exp", "fit_half_width_deg": 0.4},
...
def _clmbai_260214_peaks() -> list[tuple]:
    """ClMBAI peaks for 260214 session with validation."""
...
def _meombai_260214_peaks() -> list[tuple]:
    """MeOMBAI peaks for 260214 session with validation."""
